...
async def main():
    ...
    for i in range(num_occupants):
        ...
        asyncio.create_task(enqueue_agent(occupant_agent))
    ...
    for i in range(num_er):
        ...
        asyncio.create_task(enqueue_agent(er_agent))
        await asyncio.sleep(1)

    # Start the Building Agent
    building_agent = bms_agnt.BMSAgent("building@localhost", "password", environment)
    asyncio.create_task(enqueue_agent(building_agent))
    await asyncio.sleep(1)


    asyncio.create_task(enqueue_behaviour(building_agent, building_agent.CallER()))
    for er_agent in er_agents:
        asyncio.create_task(enqueue_behaviour(er_agent, er_agent.GoToBuilding()))
